...
public class BTreeOld<Key extends Comparable<Key>, Value> {
    ...
    /**
     * Deletes the specified key in the B-tree
    ...
    public void delete(Key key) {
        if (key == null || get(key) == null) throw new IllegalArgumentException("argument key to delete() is null ");
    ...
    // returns the last key of the right most node from current node
    private Comparable getPred(BTreeNode node, int idx) {
        BTreeNode curr = node.children[idx].next;
        while (curr != null) {
            // found a leaf
            if (curr.children[0].next == null) {
                break;
            }
        }
        return curr.children[curr.children.length - 1].key;
    }

    private void getSucc(int idx) {
